...
router.patch('/', async (req, res) => {
    ...
    try {
      const  user  =  await pool.query(`SELECT * FROM reg_users WHERE email= $1;`, [email]);
      if(user.rows.length  ===  0) {
        return res.status(404).json({ error: "Invalid email" });
      } else {
        const resetLink = jwt.sign({ id: user.rows[0].user_id, email: user.rows[0].email }, resetSecret, { expiresIn: '60m' });
        await pool.query(`UPDATE reg_users SET resetLink = $1 WHERE email = $2`, [ resetLink,email] );
